%{
	...
	#include "lex.yy.c"
	#define ID_ARRAY 100
	int vartype,count=0,count2=1,count3=-3,vartype1;
	FILE *fp;
	int reg=-1,lab=-1;
	int returntype;
	...
	void compil();
	void argcheck2(struct tnode *);
	struct gnode *head=NULL;
	struct lnode *head2=NULL;
	struct argnode *head3=NULL,*arg_arg;
%}
...
Type:	INTEGER		{vartype=INTEGER;}
	...
	;	
Type1:	INTEGER		{vartype1=INTEGER;}
	| BOOLEAN 	{vartype1=BOOLEAN;}
	;		
gidlist:	gidlist ',' gid
	| gid
	;
...
arg:	Type1 argidlist
	;				
argidlist:	argidlist ',' ID	{arginstall($3,vartype1);}
	| argidlist ',' '&' ID		{arginstall($4,vartype1);}
	| ID	{arginstall($1,vartype1);}
	| '&' ID	{arginstall($2,vartype1);}
	;
fdef:	fdef fdecl
	|
	;
fdecl:	Type ID '(' arglist2 ')' '{' ldef body '}'	{$$=makenode(vartype,FUNCTION,$2,-1,$8,NULL,NULL);
...
arglist2:	arglist2 ';' arg2	
	...
	|	
	;
arg2:	Type1 argidlist2		
	;		
argidlist2:	argidlist2 ',' ID	{linstall($3,vartype1,0,1);arginstall($3,vartype1);}	//0 for id and 1 for &id 
	| argidlist2 ',' '&' ID		{linstall($4,vartype1,1,1);arginstall($4,vartype1);}	//1 for arg and 0 for local
	| ID	{linstall($1,vartype1,0,1);arginstall($1,vartype1);}
	| '&' ID	{linstall($2,vartype1,1,1);arginstall($2,vartype1);}
	;
...
maindef:	INTEGER MAIN '(' ')' '{' ldef body '}'	{$$=makenode(INTEGER,FUNCTION,"main",-1,$7,NULL,NULL);functionGen($$);}
	;
body:	BEG slist rstmt END			{$$=makenode(VOID,3,NULL,-1,$2,NULL,$3);}
	;
...
expr:	expr '+' expr	{$$=makenode(INTEGER,'+',NULL,-1,$1,NULL,$3); typechecking($$,$$->left,$$->middle,$$->right);}
	...
	| expr '*' expr	{$$=makenode(INTEGER,'*',NULL,-1,$1,NULL,$3); typechecking($$,$$->left,$$->middle,$$->right);}
	| expr '%' expr	{$$=makenode(INTEGER,'%',NULL,-1,$1,NULL,$3); typechecking($$,$$->left,$$->middle,$$->right);}
	| ID '(' exprlist ')'	{struct gnode *k=glookup($1);if(k == NULL){yyerror();};$$=makenode(k->type,FUNCTION,$1,-1,NULL,NULL,NULL);
										$$->ArgList=$3;	
	...
	| var 	{$$=$1;}
	;
var:	ID 			{struct lnode *t=llookup($1);
								if(t == NULL){								
...
exprlist:	exprlist ',' expr	{$$=makenode(VOID,6,NULL,-1,$1,NULL,$3);}
	...
	|	{$$=NULL;}
	;	
rstmt:	RETURN expr ';'	{$$=$2;returntype=$$->type;}
	;	
%%		
...
void argcheck(struct tnode *tem){
	if(tem!=NULL){
		if(tem->nodetype==6){			
			argcheck(tem->left);
			argcheck(tem->right);
			...
			if(arg_arg == NULL){
				printf("more no of arguments\n");
				yyerror();
			}
			if(tem->type == arg_arg->type){			
				arg_arg=arg_arg->next;
			}
			/*else{
				printf("error at function call or global declaration");
				yyerror();
			}*/
		}
	}
	else if(arg_arg !=NULL){
		printf("less no of arguments %s\n",arg_arg->name);
		yyerror();
	}
}

void argcheck2(struct tnode *t){
	struct argnode *tem=head3;
	struct gnode *g=glookup(t->name);
	struct argnode *an=g->arglist;
	while(tem != NULL && an != NULL ){
		if((tem->type == an->type) && (strcmp(tem->name,an->name)==0))
		{
		tem=tem->next;
		an=an->next;}
		else{
		printf("error in defination");
		yyerror();
		}
	}
	if(tem == NULL && an == NULL)
	;
		...
		printf("error at function defination or global declaration");
		yyerror();	
	}
}
...
void typechecking(struct tnode *tree,struct tnode *le,struct tnode *midd,struct tnode *rig){
	if(tree->nodetype == '+' || tree->nodetype == '-' || tree->nodetype == '*' || tree->nodetype == '/' || tree->nodetype == '%'){
		if(le->type == INTEGER && rig->type == INTEGER)
			...
			printf("Error in Arithmetic exp %d line\n",line);
			exit(1);
			}
	}
		...
		;
			...
			printf("Error in Relational exp %d line\n",line);
			exit(1);
			}
	}
		...
		;
			...
			printf("Error in and ,or exp %d line\n",line);
			exit(1);
			}
	}
		...
		if(g == NULL){
			struct lnode *l=llookup(tree->name);
			if(l != NULL)
			;
			else{
		...
		;
			...
			printf("Error in idarray %d line\n",line);
			exit(1);
		}
	}
		...
		if(strcmp(tree->name,"main") != 0){
			...
			if(t == NULL){
				printf("Error in function %d line\n",line);
				exit(1);
			}
			else{
				arg_arg=t->arglist;
				argcheck(tree->ArgList);								
			}
		}
	}
		...
		;
			...
			printf("Error in if exp %d line\n",line);						
			exit(1);
			}
	}	
	else if(tree->nodetype == ELSE){
		if(le->type == BOOLEAN && rig->type == VOID && midd->type == VOID)
		;
		else{
			printf("Error in else exp %d line\n",line);						
			exit(1);
			}
	}
		...
		;
			...
			printf("Error in while exp %d line\n",line);
			exit(1);
			}
	}
		...
		;
			...
			printf("Error in read exp %d line\n",line);
			exit(1);
		}
	}
		...
		;
			...
			printf("Error in write exp %d line\n",line);
			exit(1);
			}
	}
		...
		;
			...
			printf("Error in = exp %d %d %dline\n",line,le->type,rig->type);
			exit(1);
			}
	}
}
